...
def fetch_all_charities():
    ...
    """
    return supabase_admin.table("Charity").select("*").execute()

def fetch_charity_by_id(user_id: str):
    """
    Fetch a single charity by userID.
    """
    return supabase_admin.table("Charity").select("*").eq("userID", user_id).single().execute()
...
def fetch_charity_profile(user_id: str):
    ...
    return supabase_admin.table("Charity") \
        .select("*, User!inner(*)") \
        .eq("userID", user_id) \
        .single() \
        .execute()
